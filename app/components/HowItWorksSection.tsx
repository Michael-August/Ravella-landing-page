...
const COMPENSATION_ITEMS = [
  ...
  ["Lifestyle Incentives", "Travel, gadgets, and elite lifestyle rewards."],
  ["Leadership Pools", "Earn from global performance pools."],
  ["Residual Earnings", "Sustainable long-term income streams."],
];

const CompensationSlider = () => {
    ...
    >
      <div ref={emblaRef} className="overflow-hidden">
        <div className="flex gap-4 sm:gap-6">
          {COMPENSATION_ITEMS.map(([title, desc], i) => (
            <motion.div
              key={title}
            ...
            >
              <div className="h-full rounded-lg sm:rounded-xl border border-gray-100 bg-white p-4 sm:p-5 lg:p-6 shadow-sm">
      ...
      <div className="mt-6 flex justify-center gap-2">
        {COMPENSATION_ITEMS.map((_, i) => (
          <button
            key={i}
            onClick={() => emblaApi?.scrollTo(i)}
            className="h-2.5 w-2.5 rounded-full bg-[#814C31]/30 hover:bg-[#814C31] transition"
          />
        ))}
      </div>
    </motion.div>
    ...
    <div className="relative w-full overflow-hidden">
      ...
      <section className="relative mt-6 sm:mt-8 lg:mt-12 rounded-[40px] sm:rounded-[50px] lg:rounded-[70px] bg-[#814C31] px-4 sm:px-6 lg:px-12 pt-10 sm:pt-12 lg:pt-16 pb-10 sm:pb-12">
        <div className="mb-10 sm:mb-12 lg:mb-16 text-center">
          ...
          <h1 className="font-bold text-white leading-tight text-2xl sm:text-3xl md:text-4xl lg:text-6xl mb-3 sm:mb-4">
            Simple. Smart. <span className="text-[#FF9801]">Automated.</span>
          </h1>

          <p className="text-[#FFFFFFB2] leading-relaxed text-sm sm:text-base md:text-lg lg:text-xl px-4">
            Five powerful steps to transform your health, income, and lifestyle—without complexity.
          </p>
        </div>

        <div className="mx-auto max-w-6xl">
          <StepsRow steps={STEPS.slice(0, 3)} cols="grid-cols-1 md:grid-cols-3" />
          <StepsRow steps={STEPS.slice(3)} cols="grid-cols-1 md:grid-cols-2 max-w-3xl mx-auto" />
        </div>
      </section>
      ...
      <section className="relative mt-6 sm:mt-8 lg:mt-12 rounded-[40px] sm:rounded-[50px] lg:rounded-[70px] bg-[#F6F0ED]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-12 py-10 sm:py-12 lg:py-16">
          <div className="mb-10 text-center">
            ...
            <h2 className="font-bold text-[#32241B] leading-tight text-2xl sm:text-3xl md:text-4xl lg:text-5xl mb-4">
              We Offer 15 Unique <br />
              <span className="bg-gradient-to-r from-[#32241B] to-[#FF9801] bg-clip-text text-transparent">
                Earning Opportunities
              </span>
            </h2>

            <p className="mx-auto max-w-2xl text-muted-foreground text-sm sm:text-base md:text-lg lg:text-xl px-4">
              Every step you take is a reward. Live Healthy with our products, Live Wealthy with our robust pay plan.
            </p>
          </div>
